// ===== Core Exercise Tracker (accordion fixed, 5 exercises restored, digital ping x2, test durations, Spotify-friendly) =====
...
// ===== Special alert: Digital Ping (5s file, play twice) =====
// Place digital_ping.mp3 in the SAME folder as core.js
...
function playDigitalPingTwice(){
  try {
    digitalPing.currentTime = 0;
    digitalPing.play();
    setTimeout(() => {
      try { digitalPing.currentTime = 0; digitalPing.play(); } catch {}
    }, 800); // short gap between pings (does not extend file length)
  } catch {}
}

// ===== UI builders =====
function buildExerciseList(){
  // Build once on init/reset
...
function advance(){
  if (!isBreak){
    ...
    // SPECIAL: end of the 3rd break (after Exercise 3, Set 3) → play ping twice
    // At this exact moment: exIdx === 2 (3rd exercise), setIdx === 2 (3rd set), isBreak === true
    if (exIdx === 2 && setIdx === SETS_PER_EXERCISE - 1) {
      playDigitalPingTwice();
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Build once (this is what restores the 5 exercises)
  buildExerciseList();
  buildDots();
  updateUI();
